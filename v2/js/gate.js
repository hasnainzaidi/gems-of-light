...
(function () {
  ...
  const gate = {
      ...
      for (const rel of Input.releases) {
        ...
        if (hit) {
          if (g.ayah === hit.i + 1) {
            ...
            // a small swirl of gold, the way the board paints a right placement
            for (let k = 0; k < 12; k++) {
              const a = (k / 12) * Math.PI * 2;
              const rr2 = 14 + (k % 3) * 7;
              this.fx.spawn('trail', hit.x + Math.cos(a) * rr2, hit.y + Math.sin(a) * rr2 * 0.7, { color: k % 3 ? '#FFE9A8' : '#F0C878' });
            }
            this.checkDone();
          } else {
    ...
    draw(ctx, W, H) {
      ...
      // courtyard floor: broad cream flagstones with a green-tile border
      ctx.save();
      ...
      // a band of little green diamond tiles where the floor begins
      const tile = mix(P.stoneDark, '#5E8F72', 0.55);
      for (let x = 10; x < W; x += 26) {
        ctx.fillStyle = alpha(tile, x % 52 < 26 ? 0.32 : 0.2);
        ctx.beginPath();
        ctx.moveTo(x, groundY + 9); ctx.lineTo(x + 6, groundY + 14); ctx.lineTo(x, groundY + 19); ctx.lineTo(x - 6, groundY + 14);
        ctx.closePath(); ctx.fill();
      }
      // the mosaic medallion between arch and settings, worn soft by feet
      const my = groundY + (H - groundY) * 0.34;
      ctx.save();
      ctx.translate(W / 2, my);
      ctx.scale(1, 0.42);
      ctx.strokeStyle = alpha(P.stoneDark, 0.28); ctx.lineWidth = 2.4;
      ctx.beginPath(); ctx.arc(0, 0, 64, 0, Math.PI * 2); ctx.stroke();
      ctx.strokeStyle = alpha(tile, 0.35); ctx.lineWidth = 1.8;
      ctx.beginPath(); ctx.arc(0, 0, 52, 0, Math.PI * 2); ctx.stroke();
      GOL.star8Path(ctx, 0, 0, 38, Math.PI / 8);
      ctx.strokeStyle = alpha(P.stoneDark, 0.3); ctx.lineWidth = 2; ctx.stroke();
      GOL.star8Path(ctx, 0, 0, 22, 0);
      ctx.fillStyle = alpha(tile, 0.22); ctx.fill();
      ctx.restore();
      ctx.restore();
      ...
      // vines trailing into the courtyard from above, as on the gate board
      for (const side of [0, 1]) {
        const baseX = side ? W - 30 : 30;
        for (let v = 0; v < 4; v++) {
          const vx = baseX + (side ? -1 : 1) * v * 26;
          const len = 46 + ((v * 37 + side * 19) % 68);
          const sway = Math.sin(t * 0.9 + v * 1.7 + side * 2) * 5;
          ctx.strokeStyle = alpha(P.leafDeep, 0.75);
          ctx.lineWidth = 1.8; ctx.lineCap = 'round';
          ctx.beginPath();
          ctx.moveTo(vx, -4);
          ctx.quadraticCurveTo(vx + sway * 0.5, len * 0.55, vx + sway, len);
          ctx.stroke();
          for (let li = 1; li <= Math.floor(len / 16); li++) {
            const ly = (len / Math.floor(len / 16)) * li - 6;
            const lx = vx + sway * (ly / len);
            GOL.paint.leafShape(ctx, lx, ly, 8, 3, (li % 2 ? 0.9 : 2.2) + sway * 0.02, alpha(li % 2 ? P.leaf : P.leafDark, 0.85));
          }
        }
      }

      // the settings bench: one carved block of cream stone, sockets waiting
      const n = this.gems.length;
      const gap2 = Math.min(84, (W - 140) / n);
      const lw = gap2 * n + 104;
      const bx = W / 2 - lw / 2, by = H * 0.84 - 36, bh = 72;
      const litCount = this.phase === 'ignite' ? this._lastLit || 0 : this.phase === 'recite' || this.phase === 'open' || this.phase === 'walk' ? n : 0;
      // settled shadow on the floor
      ctx.fillStyle = 'rgba(46,64,50,0.2)';
      ctx.beginPath(); ctx.ellipse(W / 2, by + bh + 5, lw * 0.52, 9, 0, 0, Math.PI * 2); ctx.fill();
      // plinth
      ctx.fillStyle = shade(P.stoneShade, 0.12);
      GOL.roundRect(ctx, bx - 8, by + bh - 12, lw + 16, 16, 6); ctx.fill();
      // body
      const bg2 = ctx.createLinearGradient(0, by, 0, by + bh);
      bg2.addColorStop(0, tint(P.stone, 0.42));
      bg2.addColorStop(0.35, tint(P.stone, 0.18));
      bg2.addColorStop(1, P.stoneShade);
      ctx.fillStyle = bg2;
      GOL.roundRect(ctx, bx, by, lw, bh, 12); ctx.fill();
      // sunlit top edge + carved groove
      ctx.strokeStyle = alpha(tint(P.stone, 0.7), 0.9); ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(bx + 10, by + 3); ctx.lineTo(bx + lw - 10, by + 3); ctx.stroke();
      ctx.strokeStyle = alpha(P.stoneDark, 0.35); ctx.lineWidth = 1.4;
      GOL.roundRect(ctx, bx + 7, by + 7, lw - 14, bh - 14, 8); ctx.stroke();
      // carved star rosettes on both end panels, green-tiled like the boards
      const tileG = mix(P.stoneDark, '#5E8F72', 0.55);
      for (const ex of [bx + 27, bx + lw - 27]) {
        GOL.star8Path(ctx, ex, by + bh / 2, 15, Math.PI / 8);
        ctx.fillStyle = alpha(tileG, 0.3); ctx.fill();
        ctx.strokeStyle = alpha(P.stoneDark, 0.45); ctx.lineWidth = 1.4; ctx.stroke();
        GOL.star8Path(ctx, ex, by + bh / 2, 8, 0);
        ctx.fillStyle = alpha(tint(P.stone, 0.5), 0.8); ctx.fill();
      }
      // the sockets: round recesses that fill with light
      this.sockets.forEach((s, i) => {
        const lit = i < litCount;
        const rg = ctx.createRadialGradient(s.x, s.y - 3, 2, s.x, s.y, 20);
        if (lit) {
          const p = 0.6 + 0.2 * Math.sin(t * 3 + i);
          rg.addColorStop(0, alpha('#FFF3C4', p));
          rg.addColorStop(0.7, alpha('#FFE9A8', p * 0.55));
          rg.addColorStop(1, alpha('#D9A44A', 0.25));
        } else {
          rg.addColorStop(0, alpha(shade(P.stoneDark, 0.35), 0.5));
          rg.addColorStop(1, alpha(shade(P.stoneDark, 0.1), 0.28));
        }
        ctx.fillStyle = rg;
        ctx.beginPath(); ctx.ellipse(s.x, s.y, 19, 17, 0, 0, Math.PI * 2); ctx.fill();
        // rim: shadowed above, kissed by light below
        ctx.strokeStyle = alpha(P.stoneDark, 0.5); ctx.lineWidth = 1.6;
        ctx.beginPath(); ctx.ellipse(s.x, s.y, 19, 17, 0, Math.PI * 1.05, Math.PI * 1.95); ctx.stroke();
        ctx.strokeStyle = alpha(tint(P.stone, 0.6), 0.8);
        ctx.beginPath(); ctx.ellipse(s.x, s.y, 19, 17, 0, Math.PI * 0.08, Math.PI * 0.92); ctx.stroke();
        if (lit) {
          ctx.strokeStyle = alpha('#D9A44A', 0.9); ctx.lineWidth = 1.6;
          ctx.beginPath(); ctx.ellipse(s.x, s.y, 21.5, 19.5, 0, 0, Math.PI * 2); ctx.stroke();
        }
        if (!this.gems.some((g) => g.placed === i)) {
          GOL.text(ctx, String(i + 1), s.x, s.y + 1, { size: 13, weight: '800', color: 'rgba(122,98,56,0.55)' });
        }
      });
