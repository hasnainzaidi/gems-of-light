  ...
  function updateCreatures(level, pl, dt, t) {
    for (const c of level.creatures) {
      ...
      if (c.type === 'bird') {
        ...
        // the most patient inhabitant of all
        c.x += c.dir * 3.5 * dt;
        const range = Math.min(c.range || 60, 40);
        if (c.x > c.homeX + range) c.dir = -1;
        if (c.x < c.homeX - range) c.dir = 1;
        c.facing = c.dir;
      } else if (c.type === 'rabbit') {
        const near = Math.abs(pl.x - c.x) < 130 && Math.abs(pl.y - c.y) < 90;
        if (near) c.facing = pl.x < c.x ? -1 : 1;
        if (c.hopT != null) {
          c.hopT += dt * 2.2;
          c.x += c.hopDir * 34 * dt;
          if (c.hopT >= 1) c.hopT = null;
        } else if (!near && Math.random() < dt * 0.35) {
          c.hopT = 0;
          c.hopDir = c.x > c.homeX + (c.range || 50) ? -1 : c.x < c.homeX - (c.range || 50) ? 1 : (Math.random() < 0.5 ? -1 : 1);
          c.facing = c.hopDir;
        }
      }
    }
