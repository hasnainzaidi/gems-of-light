...
(function () {
  ...
  // ================================================================ TITLE ==
  // The title wears the concept painting itself — the arrival vista from the
  // first board — with the game's own light moving over it. If the painting
  // hasn't arrived (offline first-run, headless), the painted-in-code garden
  // stands in seamlessly.
  let keyArt = null;
  if (typeof Image !== 'undefined') {
    try {
      keyArt = new Image();
      keyArt.src = 'assets/key-art.jpg';
    } catch (e) { keyArt = null; }
  }
  const keyArtReady = () => keyArt && keyArt.complete && keyArt.naturalWidth > 0;
  ...
  const title = {
    ...
    draw(ctx, W, H) {
      const t = this.t;
      const painted = keyArtReady();
      if (painted) {
        // the concept painting itself, cover-fit, drifting like a held breath
        const iw = keyArt.naturalWidth, ih = keyArt.naturalHeight;
        const zoom = 1.07 + Math.sin(t * 0.07) * 0.02;
        const s = Math.max(W / iw, H / ih) * zoom;
        const dw = iw * s, dh = ih * s;
        const dx = (W - dw) / 2 + Math.sin(t * 0.05) * 8;
        const dy = (H - dh) / 2 + Math.cos(t * 0.043) * 6;
        ctx.drawImage(keyArt, dx, dy, dw, dh);
        // a soft veil, darker above and below, so the words can rest on it
        const veil = ctx.createLinearGradient(0, 0, 0, H);
        veil.addColorStop(0, 'rgba(36,54,38,0.34)');
        veil.addColorStop(0.34, 'rgba(36,54,38,0.12)');
        veil.addColorStop(0.55, 'rgba(36,54,38,0)');
        veil.addColorStop(1, 'rgba(36,54,38,0.26)');
        ctx.fillStyle = veil;
        ctx.fillRect(0, 0, W, H);
        // the game's living light drifting over the painting
        if (Math.random() < 0.12) this.fx.spawn('sparkle', W * (0.6 + Math.random() * 0.35), H * (0.05 + Math.random() * 0.3), { color: '#FFE9A8' });
        this.fx.draw(ctx);
      } else {
      ...
      // wordmark
      const ty = H * 0.3;
      const inkCol = painted ? '#FFF7DE' : INK;
      const softCol = painted ? 'rgba(255,247,222,0.85)' : INK_SOFT;
      GOL.star8(ctx, W / 2 - 178, ty - 8, 7, Math.PI / 8, alpha(GOLD, 0.9));
      GOL.star8(ctx, W / 2 + 178, ty - 8, 7, Math.PI / 8, alpha(GOLD, 0.9));
      GOL.text(ctx, 'Gems of Light', W / 2, ty - 10, { size: Math.min(58, W * 0.07), weight: '800', color: inkCol });
      GOL.text(ctx, 'جواهر النور', W / 2, ty + 42, { size: 34, ar: true, color: painted ? '#F3CC7C' : GOLD });
      GOL.text(ctx, 'a garden of the Qur’an, one gem at a time', W / 2, ty + 84, { size: 16, weight: '600', color: softCol });
      // begin
      ...
      GOL.drawVignette(ctx, W, H, painted ? 0.16 : 0.12);
    }
  };
  ...
  const map = {
    ...
    draw(ctx, W, H) {
      ...
      for (const n of nodes) {
        ...
        // a faint carved rosette ring, like a paving medallion
        GOL.star8Path(ctx, n.x, n.y, 22.5, Math.PI / 8);
        ctx.strokeStyle = alpha(isOpen ? '#A98F5C' : '#8C8672', 0.28);
        ctx.lineWidth = 1.4;
        ctx.stroke();
        // moss settling at its base
        ctx.fillStyle = alpha('#6FA55B', isOpen ? 0.75 : 0.4);
        ctx.beginPath(); ctx.ellipse(n.x - 17, n.y + 24, 7, 3, 0.5, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.ellipse(n.x + 19, n.y + 22, 5, 2.4, -0.4, 0, Math.PI * 2); ctx.fill();
        if (isDone) {
          ...
          // a young sprout for what's still to come — Lightling's own leaf
          ctx.fillStyle = alpha('#8A7A58', 0.6);
          ctx.beginPath(); ctx.ellipse(n.x, n.y + 10, 8, 3, 0, 0, Math.PI * 2); ctx.fill();
          ctx.strokeStyle = '#7E8F6E'; ctx.lineWidth = 2.2; ctx.lineCap = 'round';
          ctx.beginPath(); ctx.moveTo(n.x, n.y + 10); ctx.quadraticCurveTo(n.x + 1, n.y + 2, n.x, n.y - 3); ctx.stroke();
          GOL.paint.leafShape(ctx, n.x, n.y - 3, 9, 3.4, -0.85, '#96A382');
          GOL.paint.leafShape(ctx, n.x, n.y - 1, 7.5, 2.8, Math.PI + 0.8, '#7E8F6E');
        }
